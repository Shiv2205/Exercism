...
pub fn row(allocator: mem.Allocator, s: []const u8, index: i32) ![]i16 {
    ...
    var input: []const u8 = s;
    for (0..@intCast(index)) |_| {
        const res = mem.cutScalar(u8, input, '\n');
        if (res == null) {
            str_row = input;
            break;
        }
    ...
    var num_row = try array_list.Aligned(i16, null).initCapacity(allocator, mem.countScalar(u8, str_row, ' ') + 1);
